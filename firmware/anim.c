...
void drawdisplay(void) {
  ...
  if (score_mode == SCORE_MODE_YEAR) {
    ...
    if (second_changed && time_s%2) {
      drawdot(GLCD_XPIXELS/2, DISPLAY_TIME_Y+VSEGMENT_H/2+4, 0);
      drawdot(GLCD_XPIXELS/2, DISPLAY_TIME_Y+VSEGMENT_H+VSEGMENT_H/2+10, 0);
    } else {
      drawdot(GLCD_XPIXELS/2, DISPLAY_TIME_Y+VSEGMENT_H/2+4, 1);
      drawdot(GLCD_XPIXELS/2, DISPLAY_TIME_Y+VSEGMENT_H+VSEGMENT_H/2+10, 1);
    }
  }
}
...
void drawsegment(uint8_t s, uint8_t x, uint8_t y, uint8_t inverted) {
  ...
  case 'a':
    ...
    break;
  case 'b':
	// b is the right top vertical segment
    drawvseg(x+HSEGMENT_W+2, y+HSEGMENT_H/2+2, inverted);
    break;
  case 'c':
	// c is the right bottom vertical segment
    drawvseg(x+HSEGMENT_W+2, y+VSEGMENT_H+4+HSEGMENT_H/2+2, inverted);
    break;
  case 'd':
	// d must be the bottom segment
    drawhseg(x+VSEGMENT_W/2+1, y+2*VSEGMENT_H+8, inverted);
    break;
  case 'e':
	// e is the left bottom vertical segment
    drawvseg(x, y+VSEGMENT_H+4+HSEGMENT_H/2+2, inverted);
    break;
  case 'f':
	// f is the left top vertical segment
    drawvseg(x,y+HSEGMENT_H/2+2, inverted);
    break;
  case 'g':
	// g is the center segment
    //drawhseg(x+VSEGMENT_W/2+1, (GLCD_YPIXELS - HSEGMENT_H)/2, inverted);
    drawhseg(x+VSEGMENT_W/2+1, y+VSEGMENT_H+4, inverted);
    break;    
  }
}
void drawvseg(uint8_t x, uint8_t y, uint8_t inverted) {
	// rectangles for all vertical segments
  glcdFillRectangle(x, y+2, VSEGMENT_W, VSEGMENT_H-4, ! inverted);

	// wider part of top triangle for vertical segments
  glcdFillRectangle(x+1, y+1, VSEGMENT_W-2, 1, ! inverted);
	// narrower part of top triangle for vertical segments
  glcdFillRectangle(x+2, y, VSEGMENT_W-4, 1, ! inverted);

	// wider part of bottom triangle for vertical segments
  glcdFillRectangle(x+1, y+VSEGMENT_H-2, VSEGMENT_W-2, 1, ! inverted);
	// narrower part of bottom triangle for vertical segments
